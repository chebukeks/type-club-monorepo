...
### Единый дизайн dropdown/popup и контекстных меню (Desktop & Web)

```
Контейнер:  bg-[var(--bg-elevated)] border border-[var(--border-strong)] rounded-xl shadow-2xl backdrop-blur-xl py-1 z-50
Пункт:      CSS-класс .menu-item  (padding: 6px 12px !important, flex, justify-content: space-between)
Hover:      .menu-item.enabled:hover → background: var(--menu-hover-bg)
Текст:      12px (text-xs, font-size: 12px !important), шорткаты и стрелки: 10px (text-[10px] text-[var(--text-dim)])
Высота:     компактная (~28px строка)
```

**Критическое правило (Защита от схлопывания отступов в десктопе):**
В среде Electron/Chromium внутри контейнеров меню со стилями `flex flex-col` **категорически запрещено полагаться на Tailwind-классы отступов** (`px-3`, `py-1.5`, `px-5`, `py-2` и т.п.) — они схлопываются или игнорируются flexbox-моделью.
Для **всех** элементов внутри меню (как стандартных строк, так и полей ввода, превью таблиц, кнопочных рядов, списков подсказок спеллчекера):
- Использовать класс `.menu-item` (содержит `padding: 6px 12px !important; font-size: 12px !important; line-height: 1.35;`).
- Для дополнительных обёрток и кастомных элементов всегда явно задавать инлайн-стиль:
```tsx
...
| POST | `/api/articles/:id/images` | Загрузить изображение в статью |
| GET | `/api/articles/:id/images` | Список изображений статьи |
| DELETE | `/api/images/:id` | Удалить изображение |

---

## Работа с изображениями, Mermaid-диаграммами и ссылками

### 1. Лайтбокс изображений и диаграмм (`ImageLightbox.ts`)
- **Расположение:** `packages/editor/src/editor/ImageLightbox.ts`.
- **Назначение:** Полноэкранный просмотрщик изображений и Mermaid-диаграмм в режимах `preview` и `read-only`.
- **Возможности управления:**
  - **Панорамирование (Pan):** Зажатая средняя кнопка мыши (**mouse3**) позволяет перетаскивать увеличенное изображение в любом направлении (аналог Figma/графических редакторов). При масштабе более 100% также работает перетаскивание левой кнопкой мыши.
  - **Масштабирование (Zoom):** Масштабирование колесиком мыши относительно точки курсора (от 10% до 1000%), кнопки панели инструментов (+, −, 1:1, Fit) и интерактивный слайдер `.type-club-lightbox-slider`.
  - **Закрытие:** Закрывается по Esc, кнопке ✕ и по клику на темный фон (с проверкой смещения курсора `dist < 5px`, исключающей случайное закрытие при панорамировании).
- **Интеграция:** Подключен в `imageView.ts` и `codeBlockView.ts` (при клике в режимах просмотра выставляется курсор `cursor: zoom-in` и вызывается `ImageLightbox.open(...)`).

### 2. Mermaid-диаграммы и перенос текста
- **Конфигурация рендерера:** В `packages/editor/src/editor/codeBlockView.ts` Mermaid инициализирован с флагами:
  ```ts
  flowchart: { useMaxWidth: true, htmlLabels: true },
  sequence: { useMaxWidth: true, wrap: true, wrapPadding: 10 }
  ```
- **Стилизация:** В `packages/editor/src/editor/editorTheme.ts` узлы `.nodeLabel`, `.edgeLabel`, `.label` снабжены правилами `white-space: normal !important; word-break: break-word !important; overflow-wrap: break-word !important;`. Текст автоматически переносится на новые строки и не выходит за границы блоков диаграммы.

### 3. Относительные ссылки (Desktop)
- **Перехват:** В `interactivePlugin.ts` относительные ссылки (без протокола, `#` и `//`) генерируют DOM-событие `editor-open-relative-link`. В вебе относительные ссылки игнорируются.
- **Обработка в Desktop:** В `apps/desktop/src/components/MarkdownEditor.tsx` ссылка разрешается относительно директории текущего файла (`window.api.dirname(currentFilePath)`):
  - Файлы `.md` / `.markdown` открываются во вкладках десктоп-редактора.
  - Файлы сторонних форматов (изображения, PDF, документы) открываются через ОС (`window.api.openExternal`).

### 4. Контекстное меню изображений
- При клике правой кнопкой мыши на изображение в режиме редактирования вызывается событие `editor-image-context-menu`.
- Включает до 5 действий с контекстным разделением:
  1. **Копировать изображение** — копирование бинарного Blob в буфер обмена системы.
  2. **Копировать ссылку** — для изображений по URL.
  3. **Встроить изображение** — преобразование URL в локальный base64 `data:` URI в документе.
  4. **Загрузить в Type Club** — отображается **только для онлайн-статей** (вызывает `POST /api/articles/:id/images`).
  5. **Сохранить локально** — отображается **только в Desktop и только для локальных файлов на диске** (сохраняет файл рядом со статьей и заменяет ссылку на относительную).

### 5. Безопасность и хранение изображений на бэкенде
- **Модель:** `ArticleImage` (`backend/app/models.py`) с внешним ключом на статью и пользователя.
- **Ограничения:**
  - Загрузка только для авторизованных авторов и соавторов онлайн-статей.
  - Максимум 10 МБ на файл.
  - Лимит 500 МБ суммарного объема картинок на одну статью.
- **Проверка содержимого:**
  - Валидация сигнатур (magic bytes) для PNG, JPEG, GIF, WEBP.
  - Глубокая проверка и санитизация SVG (блокировка `<script>`, инлайн JS-атрибутов и псевдопротоколов).
  - Защита от Path Traversal (генерация случайного UUID-имени файла).
- **Автоочистка:** В `backend/app/tasks/cleanup_images.py` реализована функция `cleanup_orphaned_images`: неиспользуемые изображения (отсутствующие в тексте статьи) автоматически удаляются с диска и из базы данных.
