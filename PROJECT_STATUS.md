...
Дата обновления: 12 сентября 2026 г.
...
### 2.10 Лайтбокс для изображений и Mermaid-диаграмм (12 сентября 2026 г.)
- **Полноэкранный оверлей (`ImageLightbox.ts`)**:
  - Реализован кроссплатформенный просмотрщик (`packages/editor/src/editor/ImageLightbox.ts`) с акриловым размытием фона (`backdrop-filter: blur(16px)`).
  - Поддерживает как растровые изображения (URL, `data:` base64, `blob:`), так и векторные SVG Mermaid-диаграммы.
  - Активируется кликом по изображению или диаграмме в режимах `preview` и `read-only` (с курсором `cursor: zoom-in`).
- **Интерактивное управление (Pan & Zoom)**:
  - Панорамирование зажатой средней кнопкой мыши (**mouse3 drag**, как в Figma) и левой кнопкой мыши при зуме >100%.
  - Масштабирование колесиком мыши относительно положения курсора (10% – 1000%).
  - Панель кнопок масштабирования: «+», «−», сброс 1:1, вписывание в экран (Fit), закрытие (✕ / Esc).
  - Слайдер масштаба `.type-club-lightbox-slider` внизу по центру с нормализацией браузерных дефолтов и интерактивностью по всей высоте дорожки и бегунка.
  - Закрытие по клику на темный фон с защитой от случайного закрытия при драге (`dist < 5px`).
- **Автоперенос текста в Mermaid**:
  - Настройка `flowchart: { useMaxWidth: true, htmlLabels: true }` и CSS `word-break: break-word` для узлов диаграммы предотвращает вылезание длинного текста за границы блоков.

### 2.11 Поддержка относительных ссылок в Desktop (12 сентября 2026 г.)
- **Диспатч события перехода**:
  - В `interactivePlugin.ts` относительные ссылки (без протокола, `#` и `//`) генерируют событие `editor-open-relative-link`. В вебе такие ссылки безопасно игнорируются.
- **Разрешение путей и открытие файлов**:
  - В `apps/desktop/electron/preload.ts` добавлены утилиты `dirname`, `joinPath`, `resolvePath`.
  - В десктопном редакторе пути разрешаются относительно директории открытого файла: `.md` файлы открываются в новой вкладке редактора, сторонние форматы — через системное приложение (`openExternal`).

### 2.12 Загрузка изображений на сервер Type Club и безопасность (12 сентября 2026 г.)
- **Бэкенд-архитектура**:
  - В `backend/app/models.py` добавлена модель `ArticleImage` с каскадным удалением при удалении статьи.
  - Реализованы эндпоинты `POST /api/articles/{article_id}/images`, `GET /api/articles/{article_id}/images`, `DELETE /api/images/{image_id}`.
- **Безопасность и лимиты**:
  - Загрузка разрешена только для онлайн-статей авторизованным авторам/соавторам статьи.
  - Лимит 10 МБ на файл и 500 МБ суммарно на статью.
  - Валидация сигнатур (magic bytes) для PNG, JPEG, GIF, WEBP.
  - Строгая проверка XML-структуры и санитизация SVG (блокировка `<script>`, событий `onload`/`onerror`, псевдопротоколов `javascript:`).
  - Защита от Path Traversal: файлы сохраняются под сгенерированными UUID.
  - Фоновая автоочистка неиспользуемых картинок (`backend/app/tasks/cleanup_images.py`): удаление файлов, не упоминаемых в тексте статьи.

### 2.13 Контекстное меню изображений в редакторе (12 сентября 2026 г.)
- **5 контекстных действий**:
  1. «Копировать изображение» — бинарный Blob в системный буфер.
  2. «Копировать ссылку» — для внешних URL.
  3. «Встроить изображение» — конвертация в base64 data URI прямо в ProseMirror AST.
  4. «Загрузить в Type Club» — доступно только для онлайн-статей (загрузка на сервер и подстановка серверного URL).
  5. «Сохранить локально» — доступно только в десктопе и только для локальных файлов на диске (сохранение в директорию документа).

### 2.14 Гармонизация отступов и типографики меню (Desktop & Web, 12 сентября 2026 г.)
- **Защита от схлопывания отступов в Electron**:
  - Класс `.menu-item` в `apps/desktop/src/index.css` переведен на `padding: 6px 12px !important; font-size: 12px !important; line-height: 1.35;`.
  - Все нестандартные элементы (инпуты, кнопки, счетчики таблиц, списки языков) снабжены явными инлайн-отступами `style={{ padding: '6px 12px' }}`.
- **Единая типографика и компактная высота строк**:
  - Размер шрифта всех контекстных меню и меню шапки в десктопе и вебе приведен к 12px (`text-xs`), подсказки шорткатов — к 10px (`text-[10px] text-gray-400`).
